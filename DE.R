                       ...
                       ptry = 100)  # number of try for Parameterization 
{
  ...
  for (randomization in 1:stry) { 
    ...
    for (parameterization in 1:ptry) {
      ...
      #lvout.2 <- stode(y = N0, fun = lv1, parms = parms, 
      #                 atol = 10^-14, rtol = 10^-12, pos = TRUE)  # 
      #lvout.3 <- ode(y = lvout.2$y, t, func = lv1, parms = parms, atol = 10^-14, rtol = 10^-12)  # 
      #      lvout.4 <- runsteady(y = N0, fun = lv1, parms = parms, times = c(0, 1e5))

      comm = jacobian.full(y = Nstar, func = lv1, parms = parms)
      lev = max(Re(eigen(comm)$values))
